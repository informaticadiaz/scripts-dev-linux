...
for i in "${!questions[@]}"; do
  ...
  if [ "$user_answer" == "$correct_answer_letter" ] || [ "$user_answer" == "${answers[$i]#* }" ]; then
    echo "Correcto!"
    score=$((score + 1))
    # Guardar la respuesta en el archivo con el resultado
    echo "Pregunta $((i+1)): Respuesta: $user_answer (Correcta)" >> "$output_file"
  else
    echo "Incorrecto. La respuesta correcta es: ${answers[$i]}"
    # Guardar la respuesta en el archivo con el resultado
    echo "Pregunta $((i+1)): Respuesta: $user_answer (Incorrecta, respuesta correcta: ${answers[$i]#* })" >> "$output_file"
  fi
  echo
done
...
if [ "$score" -ge $(( ${#questions[@]} / 2 )) ]; then
  ...
  echo "Resultado: Aprobado" >> "$output_file"
else
  echo "Desaprobaste la evaluación. ¡Sigue practicando!"
  echo "Resultado: Reprobado" >> "$output_file"
fi
